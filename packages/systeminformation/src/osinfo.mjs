...
import { stat, existsSync, readFileSync } from "fs";
import {
  noop,
  execOptsWin,
  getCodepage,
...
function getFQDN() {
  ...
  if (_linux || _darwin) {
    try {
      ...
      fqdn = stdout.toString().split(EOL)[0];
    } catch (e) {
      noop();
    }
  }
  ...
  if (_windows) {
    try {
      ...
      fqdn = stdout.toString().replace(".%USERDNSDOMAIN%", "").split(EOL)[0];
    } catch (e) {
      noop();
    }
  }
...
function osInfo(callback) {
  return new Promise((resolve) => {
    process.nextTick(() => {
        ...
        exec(
          ...
          function (error, stdout) {
            ...
            lines.forEach(function (line) {
              if (line.indexOf("=") !== -1) {
                release[line.split("=")[0].trim().toUpperCase()] = line
                  .split("=")[1]
                  .trim();
              }
            });
      ...
      if (_freebsd || _openbsd || _netbsd) {
        exec(
          "sysctl kern.ostype kern.osrelease kern.osrevision kern.hostuuid machdep.bootmethod",
          function (error, stdout) {
            if (!error) {
              let lines = stdout.toString().split("\n");
              ...
              result.codepage = getCodepage();
              result.uefi =
                getValue(lines, "machdep.bootmethod")
                  .toLowerCase()
                  .indexOf("uefi") >= 0;
            }
            if (callback) {
          ...
          function (error, stdout) {
            ...
            result.codename =
              result.release.indexOf("10.4") > -1
                ...
                : result.codename;
            result.codename =
              result.release.indexOf("10.5") > -1
                ? "Mac OS X Leopard"
                : result.codename;
            result.codename =
              result.release.indexOf("10.6") > -1
                ? "Mac OS X Snow Leopard"
                : result.codename;
            result.codename =
              result.release.indexOf("10.7") > -1
                ? "Mac OS X Lion"
                : result.codename;
            result.codename =
              result.release.indexOf("10.8") > -1
                ? "OS X Mountain Lion"
                : result.codename;
            result.codename =
              result.release.indexOf("10.9") > -1
                ? "OS X Mavericks"
                : result.codename;
            result.codename =
              result.release.indexOf("10.10") > -1
                ? "OS X Yosemite"
                : result.codename;
            result.codename =
              result.release.indexOf("10.11") > -1
                ? "OS X El Capitan"
                : result.codename;
            result.codename =
              result.release.indexOf("10.12") > -1
                ? "macOS Sierra"
                : result.codename;
            result.codename =
              result.release.indexOf("10.13") > -1
                ? "macOS High Sierra"
                : result.codename;
            result.codename =
              result.release.indexOf("10.14") > -1
                ? "macOS Mojave"
                : result.codename;
            result.codename =
              result.release.indexOf("10.15") > -1
                ? "macOS Catalina"
                : result.codename;
            result.codename = result.release.startsWith("11.")
              ? "macOS Big Sur"
              : result.codename;
            result.codename = result.release.startsWith("12.")
              ? "macOS Monterey"
              : result.codename;
            result.uefi = true;
            result.codepage = getCodepage();
      ...
      if (_windows) {
        ...
        try {
          ...
          promiseAll(workload).then((data) => {
            ...
            result.servicepack =
              getValue(lines, "ServicePackMajorVersion", ":").trim() +
              "." +
              getValue(lines, "ServicePackMinorVersion", ":").trim();
            result.codepage = getCodepage();
            ...
            const hyperv = data.results[1]
              ? data.results[1].toString().toLowerCase()
              : "";
            result.hypervisor = hyperv.indexOf("true") !== -1;
            const term = data.results[2] ? data.results[2].toString() : "";
            result.remoteSession =
              term.toString().toLowerCase().indexOf("true") >= 0;
...
const _osInfo = osInfo;
export { _osInfo as osInfo };

function isUefiLinux() {
  return new Promise((resolve) => {
    process.nextTick(() => {
      stat("/sys/firmware/efi", function (err) {
        if (!err) {
          return resolve(true);
        } else {
      ...
      try {
        if ({}.hasOwnProperty.call(appsObj.versions, "openssl")) {
          ...
          exec("openssl version", function (error, stdout) {
            if (!error) {
              ...
              appsObj.versions.systemOpenssl =
                openssl.length > 0 ? openssl[1] : openssl[0];
              appsObj.versions.systemOpensslLib =
                openssl.length > 0 ? openssl[0] : "openssl";
            }
            functionProcessed();
          });
        }
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "gulp")) {
          ...
          exec(`${cmd} --version`, function (error, stdout) {
            if (!error) {
              ...
              appsObj.versions.gulp = (
                gulp.toLowerCase().split("version")[1] || ""
              ).trim();
            }
            functionProcessed();
          });
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "tsc")) {
          ...
          exec(`${cmd} --version`, function (error, stdout) {
            if (!error) {
              ...
              appsObj.versions.tsc = (
                tsc.toLowerCase().split("version")[1] || ""
              ).trim();
            }
            functionProcessed();
          });
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "grunt")) {
          ...
          exec(`${cmd} --version`, function (error, stdout) {
            if (!error) {
              ...
              appsObj.versions.grunt = (
                grunt.toLowerCase().split("cli v")[1] || ""
              ).trim();
            }
            functionProcessed();
          });
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "git")) {
          if (_darwin) {
            const gitHomebrewExists =
              existsSync("/usr/local/Cellar/git") ||
              existsSync("/opt/homebrew/bin/git");
            if (darwinXcodeExists() || gitHomebrewExists) {
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "nginx")) {
          exec("nginx -v 2>&1", function (error, stdout) {
            if (!error) {
              ...
              appsObj.versions.nginx = (
                nginx.toLowerCase().split("/")[1] || ""
              ).trim();
            }
            functionProcessed();
          });
          ...
          exec("docker --version", function (error, stdout) {
            if (!error) {
              ...
              appsObj.versions.docker =
                parts.length > 2 && parts[2].endsWith(",")
                  ? parts[2].slice(0, -1)
                  : "";
            }
            functionProcessed();
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "postfix")) {
          exec("postconf -d | grep mail_version", function (error, stdout) {
            if (!error) {
              ...
              appsObj.versions.postfix = getValue(
                postfix,
                "mail_version",
                "=",
                true
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "mongodb")) {
          exec("mongod --version", function (error, stdout) {
            if (!error) {
              ...
              appsObj.versions.mongodb = (
                mongodb.toLowerCase().split(",")[0] || ""
              ).replace(/[^0-9.]/g, "");
            }
            functionProcessed();
          });
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "postgresql")) {
          if (_linux) {
                ...
                exec("psql -V", function (error, stdout) {
                  if (!error) {
                    ...
                    appsObj.versions.postgresql = postgresql.length
                      ? postgresql[postgresql.length - 1]
                      : "";
                    appsObj.versions.postgresql =
                      appsObj.versions.postgresql.split("-")[0];
                  }
                  functionProcessed();
                });
                functionProcessed();
            ...
            if (_windows) {
              powerShell(
                "Get-WmiObject Win32_Service | select caption | fl"
              ).then((stdout) => {
                let serviceSections = stdout.split(/\n\s*\n/);
                for (let i = 0; i < serviceSections.length; i++) {
                  if (serviceSections[i].trim() !== "") {
                    let lines = serviceSections[i].trim().split("\r\n");
                    let srvCaption = getValue(
                      lines,
                      "caption",
                      ":",
                      true
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "python")) {
          if (_darwin) {
            ...
            const gitHomebrewExists1 = existsSync("/usr/local/Cellar/python");
            const gitHomebrewExists2 = existsSync("/opt/homebrew/bin/python");
            if (
              (darwinXcodeExists() && semverCompare("12.0.1", osVersion) < 0) ||
              gitHomebrewExists1 ||
              gitHomebrewExists2
            ) {
              ...
              exec(cmd, function (error, stdout) {
                if (!error) {
                  ...
                  appsObj.versions.python = python
                    .toLowerCase()
                    .replace("python", "")
                    .trim();
                }
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "python3")) {
          if (_darwin) {
            const gitHomebrewExists =
              existsSync("/usr/local/Cellar/python3") ||
              existsSync("/opt/homebrew/bin/python3");
            if (darwinXcodeExists() || gitHomebrewExists) {
              exec("python3 -V 2>&1", function (error, stdout) {
                if (!error) {
                  const python = stdout.toString().split("\n")[0] || "";
                  appsObj.versions.python3 = python
                    .toLowerCase()
                    .replace("python", "")
                    .trim();
                }
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "pip")) {
          if (_darwin) {
            const gitHomebrewExists =
              existsSync("/usr/local/Cellar/pip") ||
              existsSync("/opt/homebrew/bin/pip");
            if (darwinXcodeExists() || gitHomebrewExists) {
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "pip3")) {
          if (_darwin) {
            const gitHomebrewExists =
              existsSync("/usr/local/Cellar/pip3") ||
              existsSync("/opt/homebrew/bin/pip3");
            if (darwinXcodeExists() || gitHomebrewExists) {
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "java")) {
          if (_darwin) {
            ...
            exec("/usr/libexec/java_home -V 2>&1", function (error, stdout) {
              if (
                !error &&
                stdout.toString().toLowerCase().indexOf("no java runtime") ===
                  -1
              ) {
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "gcc")) {
          if ((_darwin && darwinXcodeExists()) || !_darwin) {
            exec("gcc -dumpversion", function (error, stdout) {
              if (!error) {
                appsObj.versions.gcc =
                  stdout.toString().split("\n")[0].trim() || "";
              }
                ...
                exec("gcc --version", function (error, stdout) {
                  if (!error) {
                    ...
                    if (gcc.indexOf("gcc") > -1 && gcc.indexOf(")") > -1) {
                      ...
                      appsObj.versions.gcc =
                        parts[1].trim() || appsObj.versions.gcc;
                    }
                  }
                  functionProcessed();
                });
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "virtualbox")) {
          exec(getVboxmanage() + " -v 2>&1", function (error, stdout) {
            if (!error) {
            ...
            lines.forEach((line) => {
              ...
              dotnet =
                ...
                (parts[0].toLowerCase().startsWith("client") && parts.length > 2
                  ? parts[1].trim()
                  : parts[0].toLowerCase().startsWith("full") &&
                    parts.length > 2
                  ? parts[1].trim()
                  : "");
            });
          ...
          function (error, stdout) {
            if (!error) {
              try {
                ...
                if (
                  jsonObj.SPHardwareDataType &&
                  jsonObj.SPHardwareDataType.length > 0
                ) {
...
echo -n "hardware: "; cat /sys/class/dmi/id/product_uuid 2> /dev/null; echo;`;
        exec(cmd, function (error, stdout) {
          ...
          if (!result.hardware) {
            const lines = readFileSync("/proc/cpuinfo", { encoding: "utf8" })
              .toString()
              .split("\n");
      ...
      if (_freebsd || _openbsd || _netbsd) {
        exec("sysctl -i kern.hostid kern.hostuuid", function (error, stdout) {
          const lines = stdout.toString().split("\n");
          result.os = getValue(lines, "kern.hostid", ":").toLowerCase();
          result.hardware = getValue(lines, "kern.hostuuid", ":").toLowerCase();
      ...
      if (_windows) {
        ...
        powerShell(
          "Get-WmiObject Win32_ComputerSystemProduct | select UUID | fl"
        ).then((stdout) => {

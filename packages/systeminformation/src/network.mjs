import { networkInterfaces as ___networkInterfaces, EOL } from "os";
...
function getDefaultNetworkInterface() {
  ...
  let ifacenameFirst = "";
  try {
    let ifaces = ___networkInterfaces();
    ...
    if (_windows) {
      ...
      let defaultIp = "";
      const cmd = "netstat -r";
      const result = execSync(cmd, execOptsWin);
      const lines = result.toString().split(EOL);
      lines.forEach((line) => {
...
const _getDefaultNetworkInterface = getDefaultNetworkInterface;
export { _getDefaultNetworkInterface as getDefaultNetworkInterface };
...
function getMacAddresses() {
  ...
  if (_linux || _freebsd || _openbsd || _netbsd) {
    ...
    try {
      const cmd =
        "export LC_ALL=C; " +
        (pathToIp ? pathToIp + " link show up" : "/sbin/ifconfig") +
        "; unset LC_ALL";
...
const _networkInterfaceDefault = networkInterfaceDefault;
export { _networkInterfaceDefault as networkInterfaceDefault };
  ...
  for (let i in sections) {
    if ({}.hasOwnProperty.call(sections, i)) {
      if (sections[i].trim() !== "") {
        ...
        let adapterType =
          getValue(lines, "AdapterTypeID", ":") === "9" ? "wireless" : "wired";
        let ifacename = getValue(lines, "Name", ":")
          .replace(/\]/g, ")")
          .replace(/\[/g, "(");
        let iface = getValue(lines, "NetConnectionID", ":")
          .replace(/\]/g, ")")
          .replace(/\[/g, "(");
        if (
        ...
        if (netEnabled !== "") {
          const speed =
            parseInt(getValue(lines, "speed", ":").trim(), 10) / 1000000;
          nics.push({
            mac: getValue(lines, "MACAddress", ":").toLowerCase(),
            dhcp:
              getValue(linesNicConfig, "dhcpEnabled", ":").toLowerCase() ===
              "true",
            name: ifacename,
            iface,
            ...
            operstate:
              getValue(lines, "NetConnectionStatus", ":") === "2"
                ? "up"
                : "down",
            type: adapterType,
          });
...
function getWindowsNics() {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      let cmd =
        "Get-WmiObject Win32_NetworkAdapter | fl *" + "; echo '#-#-#-#';";
      cmd +=
        "Get-WmiObject Win32_NetworkAdapterConfiguration | fl DHCPEnabled" + "";
      try {
        powerShell(cmd).then((data) => {
          data = data.split("#-#-#-#");
...
function getWindowsDNSsuffixes() {
  ...
  try {
    const ipconfig = execSync("ipconfig /all", execOptsWin);
    ...
    ipconfigArray.forEach((element, index) => {
      if (index == 1) {
        ...
        const primaryDNS = longPrimaryDNS[0].substring(
          longPrimaryDNS[0].lastIndexOf(":") + 1
        );
      ...
      if (index > 1) {
        if (index % 2 == 0) {
          ...
          const connectionSpecificDNS = element
            .split("\r\n")
            .filter((element) => {
              return element.toUpperCase().includes("DNS");
            });
          const dnsSuffix = connectionSpecificDNS[0].substring(
            connectionSpecificDNS[0].lastIndexOf(":") + 1
          );
...
function getWindowsWiredProfilesInformation() {
  try {
    const result = execSync("netsh lan show profiles", execOptsWin);
    ...
    try {
      ...
      if (SSID !== "Unknown") {
        i8021xState = execSync(
          `netsh wlan show profiles "${SSID}" | findstr "802.1X"`,
          execOptsWin
        );
        i8021xProtocol = execSync(
          `netsh wlan show profiles "${SSID}" | findstr "EAP"`,
          execOptsWin
        );
      }
...
function parseLinesDarwinNics(sections) {
  ...
  sections.forEach((section) => {
    ...
    nic.type =
      getValue(section, "type").toLowerCase().indexOf("wi-fi") > -1
        ? "wireless"
        : "wired";
    nic.operstate =
      getValue(section, "status").toLowerCase().indexOf("active") > -1
        ? "up"
        : "down";
    nic.duplex =
      getValue(section, "media").toLowerCase().indexOf("half-duplex") > -1
        ? "half"
        : "full";
...
function getLinuxIfaceIEEE8021xAuth(connectionName) {
  if (connectionName) {
    ...
    try {
      const result = execSync(cmd).toString();
      const resultFormat = result.replace(/\s+/g, " ").trim();
      const authenticationProtocol = resultFormat
        .split(" ")
        .slice(1)
        .toString();
    ...
    return (
      virtualMacs.filter((item) => {
        return mac
          .toUpperCase()
          .toUpperCase()
          .startsWith(item.substr(0, mac.length));
      }).length > 0 ||
        ...
        if (JSON.stringify(ifaces) === JSON.stringify(_ifaces) && !rescan) {
          ...
          nics.forEach((nic) => {
            ...
            result.push({
              ...
              virtual: nic.internal
                ? false
                : testVirtualNic(nic.iface, nic.iface, nic.mac),
          ...
          for (let dev in ifaces) {
            ...
            if ({}.hasOwnProperty.call(ifaces, dev)) {
              ...
              ifaces[dev].forEach(function (details) {
                ...
                const nodeMainVersion = parseInt(
                  process.versions.node.split("."),
                  10
                );
                if (
                  mac.indexOf("00:00:0") > -1 &&
                  (_linux || _darwin) &&
                  !details.internal &&
                  nodeMainVersion >= 8 &&
                  nodeMainVersion <= 11
            ...
            echo -n "wirelessspeed: "; iw dev ${iface} link 2>&1 | grep bitrate; echo;`;
              ...
              mtu = parseInt(getValue(lines, "mtu"), 10);
              let myspeed = parseInt(getValue(lines, "speed"), 10);
              speed = isNaN(myspeed) ? null : myspeed;
              let wirelessspeed = getValue(lines, "wirelessspeed").split(
                "tx bitrate: "
              );
              ...
              carrierChanges = parseInt(getValue(lines, "carrier_changes"), 10);
              const operstate = getValue(lines, "operstate");
              type =
                operstate === "up"
                  ? getValue(lines, "wireless").trim()
                    ? "wireless"
                    : "wired"
                  : "unknown";
              ...
              let internal =
                ifaces[dev] && ifaces[dev][0] ? ifaces[dev][0].internal : false;
              if (
              ...
              const virtual = internal
                ? false
                : testVirtualNic(dev, ifaceName, mac);
              result.push({
                iface,
                ifaceName,
          ...
          getWindowsNics().then(function (nics) {
            ...
            for (let dev in ifaces) {
              ...
              if ({}.hasOwnProperty.call(ifaces, dev)) {
                ...
                let internal =
                  ifaces[dev] && ifaces[dev][0]
                    ? ifaces[dev][0].internal
                    : false;
                if (
                  dev.toLowerCase().indexOf("loopback") > -1 ||
                  ifaceName.toLowerCase().indexOf("loopback") > -1
...
const __networkInterfaces = networkInterfaces;
export { __networkInterfaces as networkInterfaces };
...
function calcNetworkSpeed(
  ...
  if (_network[iface] && _network[iface].ms) {
    ...
    result.rx_sec =
      rx_bytes - _network[iface].rx_bytes >= 0
        ? (rx_bytes - _network[iface].rx_bytes) / (result.ms / 1000)
        : 0;
    result.tx_sec =
      tx_bytes - _network[iface].tx_bytes >= 0
        ? (tx_bytes - _network[iface].tx_bytes) / (result.ms / 1000)
        : 0;
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (isFunction(ifaces) && !callback) {
        ...
        ifaces.__proto__.toLowerCase = stringToLower;
        ifaces.__proto__.replace = stringReplace;
        ifaces.__proto__.trim = stringTrim;
...
function networkStatsSingle(iface) {
        ...
        if (sections[i].trim() !== "") {
          ...
          perfData.push({
            name: getValue(lines, "Name", ":")
              ...
              .replace("#", "_")
              .toLowerCase(),
            rx_bytes: parseInt(getValue(lines, "BytesReceivedPersec", ":"), 10),
            rx_errors: parseInt(
              getValue(lines, "PacketsReceivedErrors", ":"),
              10
            ),
            rx_dropped: parseInt(
              getValue(lines, "PacketsReceivedDiscarded", ":"),
              10
            ),
            tx_bytes: parseInt(getValue(lines, "BytesSentPersec", ":"), 10),
            tx_errors: parseInt(
              getValue(lines, "PacketsOutboundErrors", ":"),
              10
            ),
            tx_dropped: parseInt(
              getValue(lines, "PacketsOutboundDiscarded", ":"),
              10
            ),
          });
        }
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      let ifaceSanitized = "";
      const s = isPrototypePolluted() ? "---" : sanitizeShellString(iface);
      ...
      if (
        ...
        if (_linux) {
          if (existsSync("/sys/class/net/" + ifaceSanitized)) {
            cmd =
              "cat /sys/class/net/" +
              ifaceSanitized +
              "/operstate; " +
...
const _networkStats = networkStats;
export { _networkStats as networkStats };
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
              ...
              if (line.length >= 7) {
                ...
                if (connstate) {
                  result.push({
                    ...
                    state: connstate,
                    pid:
                      proc[0] && proc[0] !== "-" ? parseInt(proc[0], 10) : null,
      ...
      if (_windows) {
        let cmd = "netstat -nao";
        try {
          exec(cmd, execOptsWin, function (error, stdout) {
            if (!error) {
              let lines = stdout.toString().split("\r\n");
...
const _networkConnections = networkConnections;
export { _networkConnections as networkConnections };
...
function networkGatewayDefault(callback) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        try {
          exec("netstat -r", execOptsWin, function (error, stdout) {
            const lines = stdout.toString().split(EOL);
            lines.forEach((line) => {
              line = line.replace(/\s+/g, " ").trim();
              if (
                line.indexOf("0.0.0.0 0.0.0.0") > -1 &&
                !/[a-zA-Z]/.test(line)
              ) {
                const parts = line.split(" ");
                if (
                  parts.length >= 5 &&
                  parts[parts.length - 3].indexOf(".") > -1
                ) {
...
const _networkGatewayDefault = networkGatewayDefault;
export { _networkGatewayDefault as networkGatewayDefault };

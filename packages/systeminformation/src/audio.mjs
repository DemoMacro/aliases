  ...
  try {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (
        lines &&
        lines.length &&
        lines[0].toLowerCase().indexOf("audio") >= 0
      ) {
        const audio = {};
        audio.slotId = lines[0].split(" ")[0];
        audio.driver =
          getValue(lines, "Kernel driver in use", ":", true) ||
          getValue(lines, "Kernel modules", ":", true);
        result.push(audio);
      }
    }
    return result;
...
function parseLinuxAudioPciMM(lines, audioPCI) {
  ...
  result.id = slotId;
  result.name = getValue(lines, "SDevice");
  // result.type = util.getValue(lines, 'Class');
  result.manufacturer = getValue(lines, "SVendor");
  result.revision = getValue(lines, "Rev");
  result.driver =
    pciMatch && pciMatch.length === 1 && pciMatch[0].driver
      ? pciMatch[0].driver
      : "";
  result.default = null;
  result.channel = "PCIe";
...
function parseDarwinAudio(audioObject, id) {
  ...
  result.driver = null;
  result.default =
    !!(audioObject.coreaudio_default_audio_input_device || "") ||
    !!(audioObject.coreaudio_default_audio_output_device || "");
  result.channel = parseDarwinChannel(channelStr);
  result.type = parseAudioType(
    result.name,
    !!(audioObject.coreaudio_device_input || ""),
    !!(audioObject.coreaudio_device_output || "")
  );
...
function parseWindowsAudio(lines) {
  ...
  result.id = getValue(lines, "DeviceID", ":"); // PNPDeviceID??
...
function audio(callback) {
  return new Promise((resolve) => {
    process.nextTick(() => {
          ...
          if (!error) {
            try {
              ...
              if (
                ...
                for (
                  let i = 0;
                  i < outObj.SPAudioDataType[0]["_items"].length;
                  i++
                ) {
                  const audio = parseDarwinAudio(
                    outObj.SPAudioDataType[0]["_items"][i],
                    i
                  );
                  result.push(audio);
                }
      ...
      if (_windows) {
        powerShell(
          "Get-WmiObject Win32_SoundDevice | select DeviceID,StatusInfo,Name,Manufacturer | fl"
        ).then((stdout, error) => {
          if (!error) {
            const parts = stdout.toString().split(/\n\s*\n/);
            for (let i = 0; i < parts.length; i++) {

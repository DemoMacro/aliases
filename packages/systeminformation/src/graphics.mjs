...
function graphics(callback) {
    ...
    try {
      graphicsArr.forEach(function (item) {
        ...
        res.controllers.push({
          vendor:
            getVendorFromModel(item.spdisplays_vendor || "") ||
            item.spdisplays_vendor ||
            "",
          ...
          vendorId:
            ...
            vendorToId(
              (item["spdisplays_vendor"] || "") + (item.sppci_model || "")
            ),
        ...
        if (item.spdisplays_ndrvs && item.spdisplays_ndrvs.length) {
          item.spdisplays_ndrvs.forEach(function (displayItem) {
            const connectionType =
              displayItem["spdisplays_connection_type"] || "";
            const currentResolutionParts = (
              displayItem["_spdisplays_resolution"] || ""
            ).split("@");
            const currentResolution = currentResolutionParts[0].split("x");
            const pixelParts = (displayItem["_spdisplays_pixels"] || "").split(
              "x"
            );
            ...
            res.displays.push({
              vendor:
                getVendorFromId(
                  displayItem["_spdisplays_display-vendor-id"] || ""
              ...
              main: displayItem["spdisplays_main"]
                ...
                : false,
              builtin:
                (displayItem["spdisplays_display_type"] || "").indexOf(
                  "built-in"
                ) > -1,
              connection:
              ...
              pixelDepth:
                pixelDepthString === "CGSThirtyBitColor"
                  ...
                  : null,
              resolutionX:
                pixelParts.length > 1 ? parseInt(pixelParts[0], 10) : null,
              resolutionY:
                pixelParts.length > 1 ? parseInt(pixelParts[1], 10) : null,
              currentResX:
                currentResolution.length > 1
                  ? parseInt(currentResolution[0], 10)
                  : null,
              currentResY:
                currentResolution.length > 1
                  ? parseInt(currentResolution[1], 10)
                  : null,
              positionX: 0,
              positionY: 0,
              currentRefreshRate:
                currentResolutionParts.length > 1
                  ? parseInt(currentResolutionParts[1], 10)
                  : null,
            });
          });
        }
    ...
    for (let i = 0; i < lines.length; i++) {
      if ("" !== lines[i].trim()) {
        if (" " !== lines[i][0] && "\t" !== lines[i][0]) {
          ...
          let _3dcontrollerpos = lines[i]
            .toLowerCase()
            .indexOf("3d controller");
          if (vgapos !== -1 || _3dcontrollerpos !== -1) {
            ...
            if (
              /[\da-fA-F]{2}:[\da-fA-F]{2}\.[\da-fA-F]/.test(pciIDCandidate)
            ) {
        ...
        if (isGraphicsController) {
          ...
          if (
            parts.length > 1 &&
            parts[0].replace(/ +/g, "").toLowerCase().indexOf("region") !==
              -1 &&
      ...
      if (device["CL_DEVICE_TYPE"] === "CL_DEVICE_TYPE_GPU") {
        ...
        if (device["CL_DEVICE_TOPOLOGY_AMD"]) {
          ...
          if (!isNaN(bus) && !isNaN(slot)) {
            ...
            const d = (slot >> 3) & 0xff;
            const f = slot & 0x07;
            busAddress = `${b.toString().padStart(2, "0")}:${d
              .toString()
              .padStart(2, "0")}.${f}`;
          }
        }
    ...
    if (_windows) {
      try {
        ...
        const candidateDirs = readdirSync(basePath).filter((dir) => {
          return readdirSync([basePath, dir].join("/")).includes(
            "nvidia-smi.exe"
          );
        });
        // use the directory with the most recently created nvidia-smi.exe file
        const targetDir = candidateDirs.reduce((prevDir, currentDir) => {
          const previousNvidiaSmi = statSync(
            [basePath, prevDir, "nvidia-smi.exe"].join("/")
          );
          const currentNvidiaSmi = statSync(
            [basePath, currentDir, "nvidia-smi.exe"].join("/")
          );
          return previousNvidiaSmi.ctimeMs > currentNvidiaSmi.ctimeMs
            ? prevDir
            : currentDir;
        });
  ...
  function parseLinesLinuxEdid(edid) {
    ...
    result.resolutionX = parseInt(
      "0x0" + edid.substr(start + 8, 1) + edid.substr(start + 4, 2)
    );
    result.resolutionY = parseInt(
      "0x0" + edid.substr(start + 14, 1) + edid.substr(start + 10, 2)
    );
    result.sizeX = parseInt(
      "0x0" + edid.substr(start + 28, 1) + edid.substr(start + 24, 2)
    );
    result.sizeY = parseInt(
      "0x0" + edid.substr(start + 29, 1) + edid.substr(start + 26, 2)
    );
    // monitor name
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if ("" !== lines[i].trim()) {
        if (
          ...
          currentDisplay.main =
            lines[i].toLowerCase().indexOf(" primary ") >= 0;
        ...
        if (
          ...
          if (
            parts1 &&
            parts1.length > 1 &&
            parts1[1].toLowerCase().indexOf("hz") >= 0
          ) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        if (isRaspberry() && isRaspbian()) {
          let cmd =
            "fbset -s | grep 'mode \"'; vcgencmd get_mem gpu; tvservice -s; tvservice -n;";
          exec(cmd, function (error, stdout) {
            ...
            if (lines.length > 1 && stdout.toString().indexOf("gpu=") >= -1) {
              result.controllers.push({
                ...
                model: "VideoCore IV",
                bus: "",
                vram: getValue(lines, "gpu", "=").replace("M", ""),
                vramDynamic: true,
              });
            }
            if (callback) {
      ...
      if (_windows) {
        ...
        try {
          const workload = [];
          workload.push(
            powerShell("Get-WmiObject win32_VideoController | fl *")
          );
          workload.push(
            powerShell(
          ...
          workload.push(
            powerShell("Get-WmiObject win32_desktopmonitor | fl *")
          );
          workload.push(
            powerShell(
          ...
          Promise.all(workload)
            .then((data) => {
              ...
              result.controllers = parseLinesWindowsControllers(
                csections,
                vsections
              );
              result.controllers = result.controllers.map((controller) => {
                ...
                if (controller.vendor.toLowerCase() === "nvidia") {
                  return mergeControllerNvidia(
                    ...
                    nvidiaData.find((device) => {
                      let windowsSubDeviceId = (
                        controller.subDeviceId || ""
                      ).toLowerCase();
                      const nvidiaSubDeviceIdParts =
                        device.subDeviceId.split("x");
              ...
              if (
                dsections.length &&
                dsections[dsections.length - 1].trim() === ""
              ) {
                dsections.pop();
              }
    ...
    for (const i in vections) {
      if ({}.hasOwnProperty.call(vections, i)) {
        if (vections[i].trim() !== "") {
          ...
          const matchingDeviceId = getValue(lines, "MatchingDeviceId").match(
            /PCI\\(VEN_[0-9A-F]{4})&(DEV_[0-9A-F]{4})(?:&(SUBSYS_[0-9A-F]{8}))?(?:&(REV_[0-9A-F]{2}))?/i
          );
          if (matchingDeviceId) {
            const quadWordmemorySize = parseInt(
              getValue(lines, "HardwareInformation.qwMemorySize")
            );
            if (!isNaN(quadWordmemorySize)) {
              let deviceId =
                matchingDeviceId[1].toUpperCase() +
                "&" +
                matchingDeviceId[2].toUpperCase();
    ...
    for (let i in sections) {
      if ({}.hasOwnProperty.call(sections, i)) {
        if (sections[i].trim() !== "") {
          ...
          let pnpDeviceId = getValue(lines, "PNPDeviceID", ":").match(
            /PCI\\(VEN_[0-9A-F]{4})&(DEV_[0-9A-F]{4})(?:&(SUBSYS_[0-9A-F]{8}))?(?:&(REV_[0-9A-F]{2}))?/i
          );
          ...
          controllers.push({
            ...
            bus: getValue(lines, "PNPDeviceID", ":").startsWith("PCI")
              ? "PCI"
              : "",
            vram:
              (memorySize == null
                ? toInt(getValue(lines, "AdapterRAM", ":"))
                : memorySize) /
              1024 /
              1024,
          ...
          _resolutionX =
            toInt(getValue(lines, "CurrentHorizontalResolution", ":")) ||
            _resolutionX;
          _resolutionY =
            toInt(getValue(lines, "CurrentVerticalResolution", ":")) ||
            _resolutionY;
          _refreshRate =
            toInt(getValue(lines, "CurrentRefreshRate", ":")) || _refreshRate;
          _pixelDepth =
            toInt(getValue(lines, "CurrentBitsPerPixel", ":")) || _pixelDepth;
        }
      }
    }
    return controllers;
  ...
  function parseLinesWindowsDisplaysPowershell(
    ...
    if (dsections && dsections.length) {
      ...
      deviceID = getValue(linesDisplay, "PNPDeviceID", ":")
        .replace(/&amp;/g, "&")
        .toLowerCase();
    ...
    for (let i = 0; i < ssections.length; i++) {
      if (ssections[i].trim() !== "") {
        ...
        const bitsPerPixel = getValue(linesScreen, "BitsPerPixel");
        const bounds = getValue(linesScreen, "Bounds")
          .replace("{", "")
          .replace("}", "")
          .replace(/=/g, ":")
          .split(",");
        const primary = getValue(linesScreen, "Primary");
        const sizeX = getValue(linesMonitor, "MaxHorizontalImageSize");
        const sizeY = getValue(linesMonitor, "MaxVerticalImageSize");
        const instanceName = getValue(
          linesMonitor,
          "InstanceName"
        ).toLowerCase();
        const videoOutputTechnology = getValue(
          linesConnection,
        ...
        displays.push({
          vendor:
            instanceName.startsWith(deviceID) && displayVendor === ""
              ? vendor
              : displayVendor,
          model:
            instanceName.startsWith(deviceID) && displayModel === ""
              ? model
              : displayModel,
          deviceName,

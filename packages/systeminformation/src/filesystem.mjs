...
const execPromiseSave = promisifySave(exec);
...
function fsSize(callback) {
  ...
  function getmacOsFsType(fs) {
    ...
    const macOsDisksSingle = macOsDisks.filter(
      (item) => item.indexOf(fsShort) >= 0
    );
    if (
      macOsDisksSingle.length === 1 &&
      macOsDisksSingle[0].indexOf("APFS") >= 0
    ) {
      return "APFS";
    }
    return "HFS";
    ...
    lines.forEach(function (line) {
      if (line !== "") {
        ...
        if (
          ...
          const size =
            parseInt(
              _linux || _freebsd || _openbsd || _netbsd ? line[2] : line[1]
            ) * 1024;
          const used =
            parseInt(
              _linux || _freebsd || _openbsd || _netbsd ? line[3] : line[2]
            ) * 1024;
          const available =
            parseInt(
              _linux || _freebsd || _openbsd || _netbsd ? line[4] : line[3]
            ) * 1024;
          const use = parseFloat(
            (100.0 * (used / (used + available))).toFixed(2)
          );
    ...
    process.nextTick(() => {
      ...
      if (_windows) {
        try {
          ...
          powerShell(
            "Get-WmiObject Win32_logicaldisk | select Caption,FileSystem,FreeSpace,Size | fl"
          ).then((stdout, error) => {
            if (!error) {
              ...
              devices.forEach(function (device) {
                ...
                if (size) {
                  data.push({
                    ...
                    available: free,
                    use: parseFloat(
                      ((100.0 * (size - free)) / size).toFixed(2)
                    ),
                    mount: caption,
                  });
                }
...
const _fsSize = fsSize;
export { _fsSize as fsSize };
    ...
    process.nextTick(() => {
      ...
      if (_freebsd || _openbsd || _netbsd || _darwin) {
        ...
        exec(cmd, { maxBuffer: 1024 * 1024 }, function (error, stdout) {
          if (!error) {
            let lines = stdout.toString().split("\n");
            result.max = parseInt(getValue(lines, "kern.maxfiles", ":"), 10);
            result.allocated =
              parseInt(getValue(lines, "kern.num_files", ":"), 10) ||
              parseInt(getValue(lines, "kern.open_files", ":"), 10);
      ...
      if (_linux) {
        readFile("/proc/sys/fs/file-nr", function (error, stdout) {
          if (!error) {
            let lines = stdout.toString().split("\n");
            if (lines[0]) {
            ...
            readFile("/proc/sys/fs/file-max", function (error, stdout) {
              if (!error) {
                let lines = stdout.toString().split("\n");
                if (lines[0]) {
...
const _fsOpenFiles = fsOpenFiles;
export { _fsOpenFiles as fsOpenFiles };
        ...
        try {
          ...
          powerShell(
            ...
            if (!error) {
              ...
              devices.forEach(function (device) {
                let lines = device.split("\r\n");
                let drivetype = getValue(lines, "drivetype", ":");
                if (drivetype) {
                  data.push({
                    name: getValue(lines, "name", ":"),
                    identifier: getValue(lines, "caption", ":"),
                    type: "disk",
                    fsType: getValue(lines, "filesystem", ":").toLowerCase(),
                    mount: getValue(lines, "caption", ":"),
                    size: getValue(lines, "size", ":"),
                    physical:
                      drivetype >= 0 && drivetype <= 6
                        ? drivetypes[drivetype]
                        : drivetypes[0],
                    uuid: getValue(lines, "volumeserialnumber", ":"),
                    label: getValue(lines, "volumename", ":"),
                    model: "",
                    serial: getValue(lines, "volumeserialnumber", ":"),
...
const _blockDevices = blockDevices;
export { _blockDevices as blockDevices };
...
function fsStats(callback) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      let wx = 0;
      if (
        (_fs_speed && !_fs_speed.ms) ||
        (_fs_speed && _fs_speed.ms && Date.now() - _fs_speed.ms >= 500)
      ) {
        if (_linux) {
...
const _fsStats = fsStats;
export { _fsStats as fsStats };
...
function calcDiskIO(rIO, wIO, rWaitTime, wWaitTime, tWaitTime) {
  ...
  if (_disk_io && _disk_io.ms) {
    ...
    result.rWaitPercent =
      ((result.rWaitTime - _disk_io.rWaitTime) * 100) / result.ms;
    result.wWaitPercent =
      ((result.wWaitTime - _disk_io.wWaitTime) * 100) / result.ms;
    result.tWaitPercent =
      ((result.tWaitTime - _disk_io.tWaitTime) * 100) / result.ms;
    _disk_io.rIO = rIO;
    _disk_io.wIO = wIO;
...
function disksIO(callback) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (
        (_disk_io && !_disk_io.ms) ||
        (_disk_io && _disk_io.ms && Date.now() - _disk_io.ms >= 500)
      ) {
...
const _disksIO = disksIO;
export { _disksIO as disksIO };
            ...
            if (!error) {
              try {
                ...
                try {
                  ...
                  if (
                    outJSON &&
                    {}.hasOwnProperty.call(outJSON, "blockdevices")
                  ) {
                ...
                devices.forEach((device) => {
                  ...
                  let interfaceType = device.tran
                    ? device.tran.toUpperCase().trim()
                    : "";
              ...
              exec(
                ...
                function (error, stdout) {
                  try {
                    ...
                    data.forEach((disk) => {
                      const diskBSDName =
                        disk.smartctl.argv[disk.smartctl.argv.length - 1];
            ...
            if (!error) {
              ...
              try {
                ...
                devices.forEach(function (device) {
                  ...
                  const BSDName = getValue(lines, "BSD Name", ":", true).trim();
                  if (sizeStr) {
                    let sizeValue = 0;
                    ...
                    if (sizeValue) {
                      const smartStatusString = getValue(
                        lines,
                        "S.M.A.R.T. status",
                        ":",
                        true
                      ...
                      result.push({
                        ...
                        vendor:
                          getVendorFromModel(
                            getValue(lines, "Model", ":", true).trim()
                          ) || getValue(lines, "Manufacturer", ":", true),
                        size: sizeValue,
                        bytesPerSector: null,
                        totalCylinders: null,
                        totalHeads: null,
              ...
              try {
                ...
                devices.forEach(function (device) {
                  ...
                  let lines = device.split("\n");
                  const linkWidth = getValue(
                    lines,
                    "link width",
                    ":",
                    true
                  ).trim();
                  const sizeStr = getValue(
                    lines,
                    "!capacity",
                    ":",
                    true
                  ...
                  if (sizeStr) {
                    ...
                    if (sizeValue) {
                      ...
                      result.push({
                        device: BSDName,
                        type: "USB",
                        name: getValue(lines, "Model", ":", true)
                          .trim()
                          .replaceAll(":", ""),
                        vendor: getVendorFromModel(
                          getValue(lines, "Model", ":", true).trim()
        ...
        try {
            ...
            try {
              ...
              if (smartDev && smartDev.devices && smartDev.devices.length > 0) {
                smartDev.devices.forEach((dev) => {
                  workload.push(
                    execPromiseSave(`smartctl -j -a ${dev.name}`, execOptsWin)
                  );
                });
              }
            } catch (e) {
          ...
          promiseAll(workload).then((data) => {
            ...
            devices.forEach(function (device) {
              ...
              const status = getValue(lines, "Status", ":")
                .trim()
                .toLowerCase();
              if (size) {
                result.push({
                  device: getValue(lines, "PNPDeviceId", ":"),
                  type: device.indexOf("SSD") > -1 ? "SSD" : "HD", // just a starting point ... better: MSFT_PhysicalDisk - Media Type ... see below
                  name: getValue(lines, "Caption", ":"),
                  vendor: getVendorFromModel(
                    getValue(lines, "Caption", ":", true).trim()
                  ),
                  size: parseInt(size),
                  bytesPerSector: parseInt(
                    getValue(lines, "BytesPerSector", ":")
                  ),
                  totalCylinders: parseInt(
                    getValue(lines, "TotalCylinders", ":")
                  ),
                  ...
                  tracksPerCylinder: parseInt(
                    getValue(lines, "TracksPerCylinder", ":")
                  ),
                  sectorsPerTrack: parseInt(
                    getValue(lines, "SectorsPerTrack", ":")
                  ),
                  firmwareRevision: getValue(
                    lines,
                    "FirmwareRevision",
            ...
            devices.forEach(function (device) {
              let lines = device.split("\r\n");
              const serialNum = getValue(lines, "SerialNumber", ":").trim();
              const name = getValue(lines, "FriendlyName", ":")
                .trim()
                .replace("Msft ", "Microsoft");
              const size = getValue(lines, "Size", ":").trim();
              const model = getValue(lines, "Model", ":").trim();
              const interfaceType = getValue(lines, "BusType", ":").trim();
              let mediaType = getValue(lines, "MediaType", ":").trim();
              ...
              if (
                ...
                (model.toLowerCase().indexOf("virtual") > -1 ||
                  model.toLowerCase().indexOf("vbox") > -1)
              ) {
                mediaType = "Virtual";
              }
              if (size) {
                    ...
                    if (i != -1) {
                      result[i].smartStatus =
                        smartData.smart_status && smartData.smart_status.passed
                          ? "Ok"
                          : smartData.smart_status &&
                            smartData.smart_status.passed === false
                          ? "Predicted Failure"
                          : "unknown";
                      if (
                        smartData.temperature &&
                        smartData.temperature.current
                      ) {
...
const _diskLayout = diskLayout;
export { _diskLayout as diskLayout };

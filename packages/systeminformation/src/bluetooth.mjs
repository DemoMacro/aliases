...
import { getValue, getFilesInPath, noop, powerShell } from "./util";
...
function parseDarwinBluetoothDevices(bluetoothObject, macAddr2) {
  ...
  result.connected =
    bluetoothObject.device_isconnected === "attrib_Yes" || false;
    ...
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        for (let i = 0; i < btFiles.length; i++) {
          const filename = basename(btFiles[i]);
          const pathParts = btFiles[i].split("/");
          const macAddr1 =
            pathParts.length >= 6 ? pathParts[pathParts.length - 2] : null;
          const macAddr2 =
            pathParts.length >= 7 ? pathParts[pathParts.length - 3] : null;
          if (filename === "info") {
            const infoFile = readFileSync(btFiles[i], {
              encoding: "utf8",
            }).split("\n");
          ...
          if (!error) {
            try {
              ...
              if (
                ...
                for (
                  let i = 0;
                  i < outObj.SPBluetoothDataType[0]["device_title"].length;
                  i++
                ) {
                  ...
                  if (objKey && objKey.length === 1) {
                    ...
                    const bluetoothDevice = parseDarwinBluetoothDevices(
                      innerObject,
                      macAddr2
                    );
      ...
      if (_windows) {
        powerShell(
          "Get-WmiObject Win32_PNPEntity | select PNPClass, Name, Manufacturer | fl"
        ).then((stdout, error) => {
          if (!error) {
            const parts = stdout.toString().split(/\n\s*\n/);
            for (let i = 0; i < parts.length; i++) {
              if (
                getValue(parts[i].split("\n"), "PNPClass", ":") === "Bluetooth"
              ) {
...
const _bluetoothDevices = bluetoothDevices;
export { _bluetoothDevices as bluetoothDevices };

...
function services(srv, callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (srv) {
        let srvString = "";
        srvString.__proto__.toLowerCase = stringToLower;
        srvString.__proto__.replace = stringReplace;
        srvString.__proto__.trim = stringTrim;

        const s = sanitizeShellString(srv);
        ...
        if (_windows) {
          try {
            ...
            powerShell(wincommand).then((stdout, error) => {
              if (!error) {
                let serviceSections = stdout.split(/\n\s*\n/);
                for (let i = 0; i < serviceSections.length; i++) {
                  if (serviceSections[i].trim() !== "") {
                    let lines = serviceSections[i].trim().split("\r\n");
                    let srvName = getValue(
                      lines,
                      "Name",
                      ":",
                      true
                    ...
                    let started = getValue(lines, "Started", ":", true);
                    let startMode = getValue(lines, "StartMode", ":", true);
                    let pid = getValue(lines, "ProcessId", ":", true);
                    if (
                      srvString === "*" ||
  ...
  function parseLine(line) {
    ...
    if (fullcommand.substr(0, 1) === "[") {
      ...
      if (firstPos === 10000 && tmpCommand.indexOf(" ") > -1) {
        ...
        if (existsSync(join(cmdPath, parts[0]))) {
          command = parts.shift();
    ...
    lines.forEach(function (line) {
      if (line.trim() !== "") {
        ...
        const pmem = parseFloat(
          ((1.0 * parseInt(parts[3]) * 1024) / totalmem()).toFixed(1)
        );
    ...
    process.nextTick(() => {
      ...
      if (
        ...
        if (_linux || _freebsd || _openbsd || _netbsd || _darwin || _sunos) {
          if (_linux) {
            cmd =
              "export LC_ALL=C; ps -axo pid:11,ppid:11,pcpu:6,pmem:6,pri:5,vsz:11,rss:11,ni:5,lstart:30,state:5,tty:15,user:20,command; unset LC_ALL";
          }
          if (_freebsd || _openbsd || _netbsd) {
            cmd =
              "export LC_ALL=C; ps -axo pid,ppid,pcpu,pmem,pri,vsz,rss,ni,lstart,state,tty,user,command; unset LC_ALL";
          }
          if (_darwin) {
            cmd =
              "ps -axo pid,ppid,pcpu,pmem,pri,vsz=xxx_fake_title,rss=fake_title2,nice,lstart,state,tty,user,command -r";
          }
          if (_sunos) {
            cmd =
              ...
              if (!error) {
                ...
                for (let i = 0; i < processSections.length; i++) {
                  if (processSections[i].trim() !== "") {
                    ...
                    let pid = parseInt(
                      getValue(lines, "ProcessId", ":", true),
                      10
                    );
                    let parentPid = parseInt(
                      getValue(lines, "ParentProcessId", ":", true),
                      10
                    );
                    let statusValue = getValue(lines, "ExecutionState", ":");
                    let name = getValue(lines, "Caption", ":", true);
                    let commandLine = getValue(lines, "CommandLine", ":", true);
                    let commandPath = getValue(
                      lines,
                      "ExecutablePath",
                      ":",
                      true
                    ...
                    let utime = parseInt(
                      getValue(lines, "UserModeTime", ":", true),
                      10
                    );
                    let stime = parseInt(
                      getValue(lines, "KernelModeTime", ":", true),
                      10
                    );
                    let memw = parseInt(
                      getValue(lines, "WorkingSetSize", ":", true),
                      10
                    );
                    ...
                    procs.push({
                      ...
                      priority: parseInt(
                        getValue(lines, "Priority", ":", true),
                        10
                      ),
                      memVsz: parseInt(
                        getValue(lines, "PageFileUsage", ":", true),
                        10
                      ),
                      memRss: Math.floor(
                        parseInt(
                      ...
                      nice: 0,
                      started: parseTimeWin(
                        getValue(lines, "CreationDate", ":", true)
                      ),
                      state: !statusValue
                        ? _winStatusValues[0]
                ...
                result.sleeping =
                  result.all - result.running - result.blocked - result.unknown;
                result.list = procs;
...
export { _processes as processes };
...
function processLoad(proc, callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      let processesString = "";
      processesString.__proto__.toLowerCase = stringToLower;
      processesString.__proto__.replace = stringReplace;
      processesString.__proto__.trim = stringTrim;

      const s = sanitizeShellString(proc);
      for (let i = 0; i <= mathMin(s.length, 2000); i++) {
        if (!(s[i] === undefined)) {
      ...
      if (isPrototypePolluted() && processesString !== "*") {
        processesString = "------";
      }
      let processes = processesString.split("|");
      let result = [];

      const procSanitized = isPrototypePolluted()
        ? ""
        : sanitizeShellString(proc);
      ...
      if (procSanitized && processes.length && processes[0] !== "------") {
        if (_windows) {
          try {
            powerShell(
              "Get-WmiObject Win32_Process | select ProcessId,Caption,UserModeTime,KernelModeTime,WorkingSetSize | fl"
            ).then((stdout, error) => {
              if (!error) {
                let processSections = stdout.split(/\n\s*\n/);
                let procStats = [];
...
const _processLoad = processLoad;
export { _processLoad as processLoad };

...
import { toInt, getValue, plistParser, powerShell } from "./util";
...
function getWifiNetworkListNmi() {
  ...
  try {
    ...
    parts.forEach((part) => {
      ...
      const frequency = getValue(lines, "FREQ")
        .toLowerCase()
        .replace("mhz", "")
        .trim();
      const security = getValue(lines, "SECURITY")
        .replace("(", "")
        .replace(")", "");
      const wpaFlags = getValue(lines, "WPA-FLAGS")
        .replace("(", "")
        .replace(")", "");
      const rsnFlags = getValue(lines, "RSN-FLAGS")
        .replace("(", "")
        .replace(")", "");
      result.push({
        ssid: getValue(lines, "SSID"),
    ...
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        if (result.length === 0) {
          try {
            ...
            for (let i = 0; i < iwconfigParts.length; i++) {
              if (
                iwconfigParts[i].indexOf("no wireless") === -1 &&
                iwconfigParts[i].trim() !== ""
              ) {
        ...
        exec(cmd, { maxBuffer: 1024 * 40000 }, function (error, stdout) {
          ...
          result = parseWifiDarwin(plistParser(output));
          if (callback) {
            callback(result);
          }
          resolve(result);
        ...
        powerShell(cmd).then((stdout) => {
          const ssidParts = stdout.toString("utf8").split(EOL + EOL + "SSID ");
          ssidParts.shift();
...
const _wifiNetworks = wifiNetworks;
export { _wifiNetworks as wifiNetworks };
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux) {
          ...
          if (ssid && bssid) {
            result.push({
              ...
              security: nmiConnection.security
                ...
                : null,
              signalLevel:
                network && network.length && network[0].signalLevel
                  ? network[0].signalLevel
                  : null,
              txRate: null,
            });
          }
          ...
          if (parts1.length > 1) {
            ...
            exec(cmd, function (error, stdout) {
              ...
              if (lines.length > 10) {
                ...
                const channel = getValue(lines2, "channel", ":", true).split(
                  ","
                )[0];
                const type = "802.11";
                ...
                if (ssid || bssid) {
                  result.push({
                    ...
                    frequency: channel
                      ? wifiFrequencyFromChannel(channel)
                      : null,
                    type,
                    security,
                    signalLevel,
        ...
        powerShell(cmd).then(function (stdout) {
          ...
          parts.forEach((part) => {
            ...
            if (lines.length >= 5) {
              const iface =
                lines[0].indexOf(":") >= 0 ? lines[0].split(":")[1].trim() : "";
              const model =
                lines[1].indexOf(":") >= 0 ? lines[1].split(":")[1].trim() : "";
              const id =
                lines[2].indexOf(":") >= 0 ? lines[2].split(":")[1].trim() : "";
              const ssid = getValue(lines, "SSID", ":", true);
              const bssid = getValue(lines, "BSSID", ":", true);
              const signalLevel = getValue(lines, "Signal", ":", true);
              const type =
                getValue(lines, "Radio type", ":", true) ||
                getValue(lines, "Type de radio", ":", true) ||
                getValue(lines, "Funktyp", ":", true) ||
                null;
              const security =
                getValue(lines, "authentication", ":", true) ||
                getValue(lines, "Authentification", ":", true) ||
                getValue(lines, "Authentifizierung", ":", true) ||
                null;
              const channel =
                getValue(lines, "Channel", ":", true) ||
                getValue(lines, "Canal", ":", true) ||
                getValue(lines, "Kanal", ":", true) ||
                null;
              const txRate =
                getValue(lines, "Transmit rate (mbps)", ":", true) ||
                getValue(lines, "Transmission (mbit/s)", ":", true) ||
                getValue(lines, "Empfangsrate (MBit/s)", ":", true) ||
                null;
...
const _wifiConnections = wifiConnections;
export { _wifiConnections as wifiConnections };
          ...
          parts.forEach((part) => {
            ...
            if (lines.length >= 5) {
              ...
              const id =
                lines[2].indexOf(":") >= 0 ? lines[2].split(":")[1].trim() : "";
              const macParts =
                lines[3].indexOf(":") >= 0 ? lines[3].split(":") : [];
              macParts.shift();
...
const _wifiInterfaces = wifiInterfaces;
export { _wifiInterfaces as wifiInterfaces };

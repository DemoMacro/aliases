...
import osxTempImport from "osx-temperature-sensor";
      ...
      cpuFlags().then((flags) => {
        ...
        if (_windows) {
          try {
            ...
            workload.push(
              util.powerShell(
                "Get-WmiObject Win32_CacheMemory | select CacheType,InstalledSize,Level | fl"
              )
            );
    ...
    process.nextTick(() => {
      ...
      if (_windows) {
        try {
          util
            .powerShell(
              "Get-WmiObject Win32_processor | select L2CacheSize, L3CacheSize | fl"
            )
            .then((stdout, error) => {
              if (!error) {
                let lines = stdout.split("\r\n");
                ...
                .then((stdout, error) => {
                  if (!error) {
                    ...
                    parts.forEach(function (part) {
                      ...
                      const cacheType = util.getValue(lines, "CacheType");
                      const level = util.getValue(lines, "Level");
                      const installedSize = util.getValue(
                        lines,
                        "InstalledSize"
                      );
                      // L1 Instructions
      ...
      if (now >= 200) {
        ...
        for (let i = 0; i < _corecount; i++) {
          ...
          _cpus[i].totalTick =
            ...
            _cpus[i].idle;
          _cpus[i].totalLoad =
            _cpus[i].user + _cpus[i].sys + _cpus[i].nice + _cpus[i].irq;
        ...
        let totalTick =
          totalUser + totalSystem + totalNice + totalIrq + totalIdle;

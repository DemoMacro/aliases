...
import { getValue, toInt, noop, powerShell } from "./util";
...
function mem(callback) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux) {
        readFile("/proc/meminfo", function (error, stdout) {
          if (!error) {
          ...
          function (error, stdout) {
            if (!error) {
              ...
              const pagesize = parseInt(
                getValue(lines, "vm.stats.vm.v_page_size"),
                10
              );
              const inactive =
                parseInt(getValue(lines, "vm.stats.vm.v_inactive_count"), 10) *
                pagesize;
              const cache =
                parseInt(getValue(lines, "vm.stats.vm.v_cache_count"), 10) *
                pagesize;

              result.total = parseInt(getValue(lines, "hw.realmem"), 10);
              if (isNaN(result.total)) {
                result.total = parseInt(getValue(lines, "hw.physmem"), 10);
              }
              result.free =
                parseInt(getValue(lines, "vm.stats.vm.v_free_count"), 10) *
                pagesize;
                  ...
                  if (lines.length > 0) {
                    ...
                    for (let i = 0; i < line.length; i++) {
                      if (line[i].toLowerCase().indexOf("total") !== -1) {
                        result.swaptotal =
                          parseFloat(line[i].split("=")[1].trim()) *
                          1024 *
                          1024;
                      }
                ...
                if (parseInt(getValue(lines, "Size"), 10) > 0) {
                  ...
                  result.push({
                    ...
                    manufacturer: getManufacturerLinux(
                      getValue(lines, "Manufacturer:")
                    ),
                    partNum: getValue(lines, "Part Number:"),
                    serialNum: getValue(lines, "Serial Number:"),
                    voltageConfigured:
                      parseFloat(getValue(lines, "Configured Voltage:")) ||
                      null,
                    voltageMin:
                      parseFloat(getValue(lines, "Minimum Voltage:")) || null,
                    voltageMax:
                      parseFloat(getValue(lines, "Maximum Voltage:")) || null,
                  });
                } else {
                  result.push({
                    size: 0,
            ...
            if (!result.length) {
              ...
              try {
                ...
                if (
                  ...
                  let freq =
                    parseInt(getValue(lines, "sdram_freq", "=", true), 10) || 0;
                  if (freq) {
                  ...
                  lines = stdout.toString().split("\n");
                  let voltage =
                    parseFloat(getValue(lines, "volt", "=", true)) || 0;
                  if (voltage) {
          ...
          powerShell(
            ...
            if (!error) {
              ...
              devices.forEach(function (device) {
                ...
                const size =
                  parseInt(getValue(lines, "Capacity", ":"), 10) || 0;
                if (size) {
                  result.push({
                    size,
                    bank: getValue(lines, "BankLabel", ":"), // BankLabel
                    type: memoryTypes[
                      parseInt(getValue(lines, "MemoryType", ":"), 10) ||
                        parseInt(getValue(lines, "SMBIOSMemoryType", ":"), 10)
                    ],
                    ecc:
                      dataWidth && totalWidth ? totalWidth > dataWidth : false,
                    clockSpeed:
                      parseInt(
                        getValue(lines, "ConfiguredClockSpeed", ":"),
                        10
                      ) ||
                      parseInt(getValue(lines, "Speed", ":"), 10) ||
                      0,
                    formFactor:
                      FormFactors[
                        parseInt(getValue(lines, "FormFactor", ":"), 10) || 0
                      ],
                    ...
                    voltageConfigured:
                      (parseInt(
                        getValue(lines, "ConfiguredVoltage", ":"),
                        10
                      ) || 0) / 1000.0,
                    voltageMin:
                      (parseInt(getValue(lines, "MinVoltage", ":"), 10) || 0) /
                      1000.0,
                    voltageMax:
                      (parseInt(getValue(lines, "MaxVoltage", ":"), 10) || 0) /
                      1000.0,
                  });
                }
              });
...
const _memLayout = memLayout;
export { _memLayout as memLayout };

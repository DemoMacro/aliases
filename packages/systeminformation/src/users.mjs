...
import { powerShell, getValue } from "./util";
    ...
    if (line === "---") {
      ...
      if (is_whopart) {
        result_who.push({
          ...
          time: l[3],
          ip:
            l && l.length > 4 ? l[4].replace(/\(/g, "").replace(/\)/g, "") : "",
        });
      } else {
        // w part
        if (w_first) {
          ...
          who_line = result_who.filter(function (obj) {
            return (
              obj.user.substring(0, 8).trim() === result_w.user &&
              obj.tty === result_w.tty
            );
          });
  ...
  lines.forEach(function (line) {
    if (line === "---") {
      ...
      if (is_whopart) {
        ...
        who_line = result_who.filter(function (obj) {
          return (
            obj.user === result_w.user &&
            (obj.tty.substring(3, 1000) === result_w.tty ||
              obj.tty === result_w.tty)
          );
        });
        if (who_line.length === 1) {
    ...
    process.nextTick(() => {
      ...
      if (_windows) {
        try {
          ...
          // ).then(data => {
          let cmd =
            "Get-WmiObject Win32_LogonSession | select LogonId,StartTime | fl" +
            "; echo '#-#-#-#';";
          cmd +=
          ...
          powerShell(cmd).then((data) => {
            ...
            if (data) {
              ...
              let loggedons = parseWinLoggedOn(
                (data[1] || "").split(/\n\s*\n/)
              );
              let queryUser = parseWinUsersQuery((data[3] || "").split("\r\n"));
              let users = parseWinUsers(
                (data[2] || "").split(/\n\s*\n/),
                queryUser
              );
              for (let id in loggedons) {
                if ({}.hasOwnProperty.call(loggedons, id)) {
                  loggedons[id].dateTime = {}.hasOwnProperty.call(sessions, id)
                    ? sessions[id]
                    : "";
                }
              }
              users.forEach((user) => {
                ...
                for (let id in loggedons) {
                  if ({}.hasOwnProperty.call(loggedons, id)) {
                    if (
                      loggedons[id].user === user.user &&
                      (!dateTime || dateTime < loggedons[id].dateTime)
                    ) {
                ...
                result.push({
                  user: user.user,
                  tty: user.tty,
                  date: `${dateTime.substr(0, 4)}-${dateTime.substr(
                    4,
                    2
                  )}-${dateTime.substr(6, 2)}`,
...
function parseWinLoggedOn(loggedonParts) {
  ...
  loggedonParts.forEach((loggedon) => {
    ...
    const domain =
      domainParts.length > 1 ? domainParts[1].replace(/"/g, "") : "";
  ...
  if (header) {
    ...
    for (let i = 1; i < lines.length; i++) {
      if (lines[i].trim()) {
        const user =
          lines[i]
            .substring(headerDelimiter[0] + 1, headerDelimiter[1])
            .trim() || "";
        const tty =
          lines[i]
            .substring(headerDelimiter[1] + 1, headerDelimiter[2] - 2)
            .trim() || "";
